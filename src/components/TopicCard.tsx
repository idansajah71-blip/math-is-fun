...
export default function TopicCard({ slug, title, level, description, index }: {
  ...
  const dot = { smp: "bg-emerald-500", sma: "bg-[#1a73e8]", kuliah: "bg-purple-500" }[level];
  ...
  return (
    <Link href={`/topic/${slug}`} className="block group">
      <div className={`flex items-center gap-4 px-5 py-3.5 rounded-xl border transition-all duration-200 ${
        isCompleted ? "bg-[#f0fdf4] dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-800" : "bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 hover:border-[#1a73e8]/30 hover:shadow-md"
      }`}>
        <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${isCompleted ? "bg-emerald-100 dark:bg-emerald-900/30" : "bg-gray-100 dark:bg-gray-800"}`}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <h3 className="text-[13px] font-semibold text-gray-900 dark:text-gray-100 group-hover:text-[#1a73e8] truncate">{title}</h3>
          </div>
